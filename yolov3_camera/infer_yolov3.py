...
def parser_outputs(outputs: List[List[np.ndarray]], orig_hws: List[np.ndarray],
                   ...
                   class_num: int, org_out_hw: List[np.ndarray], org_in_hw: List[np.ndarray],
                   xy_offsets: List[tf.Tensor], anchors: List[np.ndarray]
    ...
    for y_pred, orig_hw in zip(outputs, orig_hws):
        ...
        y_pred = [np.reshape(pred, list(pred.shape[:-1]) + [anchor_number,
                                                            5 + class_num])
                  for pred in y_pred]
        """ box list """
        _xyxy_box = []
        ...
        for l, pred_label in enumerate(y_pred):
            ...
            pred_xy_A, pred_wh_A = xywh_to_all(pred_xy, pred_wh, org_out_hw[l], xy_offsets[l], anchors[l])
...
def draw_image(img: np.ndarray, ann: np.ndarray, is_show=True, scores=None) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    iou_method = 'diou'
    class_num = 20
    xy_offsets: List[tf.Tensor] = [calc_xy_offset(org_out_hw[i]) for i in range(len(org_out_hw))]
    ...
    while(True):
        ...
        det_img, _ = resize_img(cv2.cvtColor(draw_img, cv2.COLOR_BGR2RGB), org_in_hw, np.zeros([0, 5], 'float32'))
        ...
        results = parser_outputs(outputs, orig_hws, obj_thresh, nms_thresh, iou_method, class_num,
                                 org_out_hw, org_in_hw, xy_offsets, anchors)
        bbox, cals, scores = results[0]
        draw_img = draw_image(draw_img, np.hstack([cals[:, None], bbox]), scores=scores)
